...
const CATEGORIES = [
  { label: 'Todos',       color: '#6B7280' },
  { label: 'Restaurante', color: '#E76F51' },
  { label: 'Museo',       color: '#457B9D' },
  { label: 'Café',        color: '#795548' },
  { label: 'Artesanías',  color: '#F4A261' },
  { label: 'Histórico',   color: '#2A9D8F' },
  { label: 'Evento',      color: '#9B59B6' },
]

export default function Drawer({ isOpen, onClose, savedPlaces = [], onCategorySelect }) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-[#2A9D8F]">MiCiudad</p>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          ...
          {/* ── Eventos cerca de mí ─────────────────── */}
          <Section icon={<CalendarIcon />} title="Eventos cerca de mí">
            {events.length === 0 ? (
              <EmptyState text="No hay eventos próximos en tu zona" />
            ) : (
              <div className="flex flex-col gap-2">
                {events.map(ev => (
                  <div key={ev.id} className="bg-gray-50 rounded-xl p-3">
                    <p className="text-sm font-semibold text-gray-800 leading-snug">{ev.nombre}</p>
                    {ev.direccion && (
                      <p className="text-xs text-gray-500 mt-0.5 leading-snug">{ev.direccion}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          {/* ── Categorías ──────────────────────────── */}
          <Section icon={<GridIcon />} title="Categorías">
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIES.map(({ label, color }) => (
                <button
                  key={label}
                  onClick={() => handleCategory(label)}
                  className="py-2.5 px-3 rounded-xl text-sm font-semibold text-left transition-all active:scale-95"
                  style={{
                    background: `${color}14`,
                    color,
...
function EmptyState({ text }) {
  return (
    <p className="text-sm text-gray-400 leading-relaxed py-1">{text}</p>
  )
}
